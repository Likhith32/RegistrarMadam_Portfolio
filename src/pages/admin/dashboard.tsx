...
export default function AdminDashboard() {
  ...
  // Loading skeleton with glassmorphic style
  if (checking) {
    return (
      <AdminLayout>
        <div className="mb-8 h-9 w-56 animate-pulse rounded-2xl bg-white/40 backdrop-blur-sm" />
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 7 }).map((_, i) => (
            <motion.div
              ...
              className="h-36 animate-pulse rounded-2xl bg-white/40 backdrop-blur-sm border border-white/30"
            />
          ))}
        </div>
      </AdminLayout>
